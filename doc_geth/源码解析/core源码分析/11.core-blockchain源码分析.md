...
### reorgs
`reorgs`是在新的链的总难度大于本地链的总难度的情况下，需要用新的区块链来替换本地的区块链为规范链。
```go
// reorg函数接受两个块，一个旧链和一个新链，将重构块并将其插入新的规范链中，并累积可能丢失的交易并发布有关它们的事件。
// 注意，新的头块不会在此处进行处理，调用者需要在外部处理它。
func (bc *BlockChain) reorg(oldHead *types.Header, newHead *types.Block) error {
	...
	if oldBlock == nil {
		return errors.New("当前头块丢失")
	}
	newBlock := newHead

	// 将较长的链缩短到与较短的链相同的数量
	if oldBlock.NumberU64() > newBlock.NumberU64() {
		// 旧链更长，收集所有交易和日志作为删除的交易
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		...
		logFn(msg, "编号", commonBlock.Number(), "哈希", commonBlock.Hash(),
			"删除", len(oldChain), "删除自", oldChain[0].Hash(), "添加", len(newChain), "添加自", newChain[0].Hash())
		blockReorgAddMeter.Mark(int64(len(newChain)))
		blockReorgDropMeter.Mark(int64(len(oldChain)))
		blockReorgMeter.Mark(1)
	} else if len(newChain) > 0 {
		// 特殊情况发生在合并阶段，当前头是新头的祖先，但这两个块不是连续的
		log.Info("扩展链", "添加", len(newChain), "编号", newChain[0].Number(), "哈希", newChain[0].Hash())
		blockReorgAddMeter.Mark(int64(len(newChain)))
	} else {
		// len(newChain) == 0 && len(oldChain) > 0
		// 将规范链倒回到较低的位置。
		log.Error("不可能的重组，请提交问题报告", "旧编号", oldBlock.Number(), "旧哈希", oldBlock.Hash(), "旧块数", len(oldChain), "新编号", newBlock.Number(), "新哈希", newBlock.Hash(), "新块数", len(newChain))
	}
	// 插入新链（除头块之外的块，逆序），
	// 注意正确的增量顺序。
	...
	// 立即删除无用的索引，包括非规范的事务索引和在头之上的规范链索引。
	...
	// 删除所有不属于新的规范链的哈希标记。
	// 因为重组函数不处理新链头，所有大于或等于新链头的哈希标记应该被删除。
	...
	if err := indexesBatch.Write(); err != nil {
		log.Crit("删除无用索引失败", "错误", err)
	}

	// 发送旧规范链的日志和块的事件，以及新规范链的'重生'日志的事件。日志数量可能非常大，因此事件以大约512个为一批发送。
