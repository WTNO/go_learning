...
经过一系列的检查和握手之后， 循环的调用了handleMsg方法来处理事件循环。 这个方法很长，主要是处理接收到各种消息之后的应对措施。（没看懂）
```go
...
// eth/protocols/snap/handle.go
// 在从远程节点接收到"snap"协议的入站消息时被调用。
// 如果返回任何错误，远程连接将被关闭。
func HandleMessage(backend Backend, peer *Peer) error {
